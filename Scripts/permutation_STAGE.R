permutation_STAGE <-
  function(Input_data) {
    ...
    mean_diff_MV_STAGE <-
      abs(mean(Input_data$MV_abs[Input_data$Variable_1 ==
                                           "adult"]) -
            mean(Input_data$MV_abs[Input_data$Variable_1 ==
                                             "nimph"]))
    median_diff_MV_STAGE <-
      abs(median(Input_data$MV_abs[Input_data$Variable_1 ==
                                             "adult"]) -
            median(Input_data$MV_abs[Input_data$Variable_1 ==
                                               "nimph"]))
    
    set.seed(2020)
    N <- length(Input_data$Variable_1)
    P <- 100000
    responses <- Input_data$MV_abs
